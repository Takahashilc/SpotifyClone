import { Component, OnDestroy } from '@angular/core';
import { Router } from '@angular/router';
import {
  faGuitar,
  faHome,
  faMusic,
  faSearch,
} from '@fortawesome/free-solid-svg-icons';
import { Subscription } from 'rxjs';
import { IPlaylist } from 'src/app/interfaces/IPlaylist';
import { IUsuario } from 'src/app/interfaces/IUsuario';
import { SpotifyService } from 'src/app/services/spotify.service';

@Component({
  selector: 'app-painel-esquerdo',
  templateUrl: './painel-esquerdo.component.html',
  styleUrls: ['./painel-esquerdo.component.scss'],
})
export class PainelEsquerdoComponent implements OnDestroy {
  menuSelecionado = 'Home';
  usuario: IUsuario | null;

  playlists: IPlaylist[] = [];

  subs: Subscription[] = [];

  //Icones
  homeIcone = faHome;
  pesquisarIcone = faSearch;
  artistaIcone = faGuitar;
  playlistIcone = faMusic;

  constructor(private router: Router, private spotifyService: SpotifyService) {
    const usuarioChange: Subscription = this.spotifyService.usuario$.subscribe(
      (usuario) => {
        if (usuario) {
          this.usuario = usuario;

          this.buscarPlaylists(usuario.id);
        }
      }
    );

    this.subs.push(usuarioChange);
  }

  ngOnDestroy(): void {
    this.subs.forEach((sub) => sub.unsubscribe());
  }

  botaoClick(botao: string) {
    this.menuSelecionado = botao;
    this.router.navigateByUrl('player/home');
  }

  botaoSearch(botao: string) {
    this.menuSelecionado = botao;
    this.router.navigateByUrl('player/pesquisar');
  }

  irParaPlaylist(playlistId: string) {
    this.menuSelecionado = playlistId;
    this.router.navigateByUrl(`player/lista/playlist/${playlistId}`);
  }

  async buscarPlaylists(id: string) {
    this.playlists = await this.spotifyService.buscarPlaylistUsuario(id);
  }
}
